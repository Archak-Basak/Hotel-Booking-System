...
if(isset($_GET['filter_mode']))
{
	...
	if(isset($_GET['update']) || $flag==1 || $ed>0)
	{
       ...
       <table cellspacing="0"  class="table_h" id="table_n" width="1098">
        <?php 
			if(isset($_GET['filter']))
			{
				?>
				 <tr bgcolor="0078d7">
         <td width="100"></td>
        <td width="100" height="44" align="center" ><select name="floor_select_f" id="filter_floor" class="cmb2"><?php echo show_floor_info_combo($conn);?></select></td>
        ...
        <td  width="187" align="center"><p class="header" ><select name="alias_select_f" id="filter_alias" class="cmb"><?php echo show_alias_combo($conn,$db);?></select></p></td>
        <td width="171" align="center" ><p class="header">No of rooms</p></td>
        <td width="140"><p class="header">No of Booked rooms</p></td>
		...
		?>
		 ...
		 else
		 {
		  ?>
           <tr bgcolor="#0078d7" onDblClick="filter()">
           <?php }?>
         <td width="100" ><input type="checkbox" id="master_check" onclick="master_check_all()"></td>
        <td width="100" height="44" align="center" bgcolor='<?php echo $color_floor;?>'><p class="header">Floor no</p></td>
         <td  width="150" align="center" bgcolor='<?php echo $color_status;?>'><p class="header">Status</p></td>
        <td  width="187" align="center" bgcolor='<?php echo $color_alias;?>'><p class="header">Alias</p></td>
        <td width="171" align="center" bgcolor='#0078d7'><p class="header">No of rooms</p></td>
        <td width="140"><p class="header">No of Booked rooms</p></td>
        ...
        while($data=mysqli_fetch_assoc($result1))
        {
			$i++;
                        ...
                        <td width="100"></td>
                            <td class="td" height="49" align="center" width="100"><input type='hidden' name='floor_id_e' value='<?php echo $data['floor_id'];?>' id='floor_id_e'>
                            ...
                            <td align="center" width="157"><select name='status_e'  class="cmb" id='status_e'>
                            ...
                            <td align="center" class="td" width="187"><input type="text" name="alias_e" id="alias_e"  size='20' class="txt220" REQUIRED value='<?php echo $data['floor_name'];?>' onKeyUp="validate_floor_edit('<?php echo $data['floor_id'];?>')"></td> 
                             <td width="171">
		 ...
		 // echo $tq;
		  ...
		  $tq="select count(*) as n_r from room_info,floor_info where room_info.floor_no=floor_info.floor_no and floor_info.floor_no='$data[floor_no]' and room_info.status='0'";
		 // echo $tq;
		  $result_n=mysqli_query($conn,$tq);
		  $dt=mysqli_fetch_assoc($result_n);
		  echo $dt['n_r'];
		  ?></td>
                          <td align="center" class="td" width="125"><input type='button'  value='Cancel' class="button_r" onClick="update_content()" ></td>
        ...
        <td height="50" width="100" align="center" class="td"><?php echo $data['floor_no'];?></td> 
             <?php 
         if($data['floor_status']==0)
         	$st='Booked';
         else if($data['floor_status']==1)
         	$st='Available';
      		 ...
      		 $st='Unavailable';
         ?>    
          <td align="center" class="td" width="157"><?php echo $st; ?></td>
          <td align="center" class="td" width="187"><?php echo $data['floor_name']; ?></td>
		 ...
		 // echo $tq;
		  ...
		  ?></td>
				 ...
				 if($n_r==0)
				 {
					 ...
					 {
				 ?>         
        <td align="center" class="td" width="125"><input type="button" class="button_r" value="Delete" onClick="try_delete('<?php echo $data['floor_id'];?>')" ></td>
       <?php 
					 }else
					 {
						 
						 ?>
						  <td align="center" class="td" width="125"><input type="button" class="button_r" value="Delete" onClick="try_delete('<?php echo $data['floor_id'];?>')" disabled ></td>
